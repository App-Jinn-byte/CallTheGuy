//
//  AppointmentCOntractorViewController.swift
//  Call The Guy
//
...
class AppointmentCOntractorViewController: UIViewController,UITableViewDelegate,UITableViewDataSource {
    ...
    fileprivate func APIRequestCompleted(response:Any?,error:Error?){
        ...
        if APIResponse.isValidResponse(viewController: self, response: response, error: error){
            ...
            do {
                ...
                if (appoint.AppointmentsList.count == 0){
    ...
    @IBAction func addAppointment(_ sender: Any) {
        let vc = UIStoryboard.init(name: "Contracter", bundle: Bundle.main).instantiateViewController(withIdentifier: "Appointment") as! CreateContracterAppointmentViewController
        navigationController!.pushViewController(vc, animated: true)
    }
}
